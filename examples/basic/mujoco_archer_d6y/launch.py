...
def get_node_cfgs(node_params_dict: dict = NODE_PARAMS_DICT):
    node_srv = node_params_dict.get(
        'mujoco_archer_d6y_srv',
        NODE_PARAMS_DICT['mujoco_archer_d6y_srv'],
    )
    node_cli = node_params_dict.get(
        'mujoco_archer_d6y_cli',
        NODE_PARAMS_DICT['mujoco_archer_d6y_cli'],
    )
    return HexNodeConfig([
        {
            "name": node_cli['name']
            if 'name' in node_cli else "mujoco_archer_d6y_cli",
            "node_path":
        ...
        {
            "name": node_srv['name']
            if 'name' in node_srv else "mujoco_archer_d6y_srv",
